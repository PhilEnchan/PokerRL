...
class _LutGetterBase:
    ...
    def get_n_boards_LUT(self):
        ...
        n_dealt_so_far = 0 
        return {
            r: comb(N=self.rules.N_RANKS * self.rules.N_SUITS - n_dealt_so_far, k=_c[r], exact=True, repetition=False)
            for r in self.rules.ALL_ROUNDS_LIST
            if (r==0 or (n_dealt_so_far := n_dealt_so_far + _c[r-1]) or True)  # 更新已发牌数
        }
